...
from maubot import MessageEvent  # type: ignore
...
class FederationBot(Plugin):
    ...
    async def start(self) -> None:
        ...
        self.federation_handler = FederationHandler(self.http, self.log, self.server_signing_keys)

    async def pre_stop(self) -> None:
        self.client.remove_event_handler(
            EventType.REACTION, self.react_control_handler  # pylint: disable=no-member
        )

    async def react_control_handler(self, react_evt: ReactionEvent) -> None:
        reaction_data = react_evt.content.relates_to
        if react_evt.sender != self.client.mxid and reaction_data.event_id in self.task_control:
    ...
    async def context_subcommand(
        ...
        stuff = await self.client.get_event_context(
            room_id=RoomID(str(room_id_or_alias)),
            event_id=EventID(str(event_id)),
            limit=int(str(limit)),
        )
    ...
    async def room_walk_command(
        ...
        if get_domain_from_id(command_event.sender) != get_domain_from_id(self.client.mxid):
            await command_event.reply(
                "I'm sorry, running this command from a user not on the same server is not allowed"
            )
            return
        ...
        try:
            per_iteration_int = int(str(per_iteration))
        except ValueError:
        ...
        async def _inner_walking_fetcher(for_direction: PaginationDirection, queue: Queue) -> None:
            retry_token = False
            back_off_time = 0.0
            next_token = None
            while True:
                ...
                try:
                    ...
                    worker_response = await self.client.get_messages(
                        room_id=room_to_check,  # type: ignore
                    ...
                    # Don't want this behind a 'finally', it should only run if not retrying request
                    queue.task_done()
        ...
        while True:
            ...
            discovery_lines.append(
                f"  Time taken: {discovery_cumulative_iter_time:.3f} seconds "
                + f"(iter# {discovery_iterations})"
            )
        ...
        while True:
            ...
            backwalk_lines.append(
                f"  Time taken: {backwalk_cumulative_iter_time:.3f} seconds "
                + f"(iter# {backwalk_iterations})"
            )
    ...
    async def find_event_command(
        ...
        host_to_event_status_map: Dict[str, EventBase | None] = {}
    ...
    async def _delegations(
        ...
        if not list_of_servers_to_check:
            try:
                joined_members = await self.client.get_joined_members(room_to_check)  # type: ignore
        ...
        tasks = []
        for i in range(  # pylint: disable=unused-variable
        ...
        for server_name, server_results in server_to_server_data.items():
            ...
            if server_results.headers is not None and not isinstance(
                server_results, FederationErrorResponse
            ):
        ...
        for server_name in server_results_sorted:
            ...
            if server_response:
                ...
                if server_response.headers is not None and not isinstance(
                    server_response, FederationErrorResponse
                ):
                    error_reason = None
    ...
    async def state_command(
        ...
        await command_event.respond(f"Retrieving {len(pdu_list)} events from {destination_server}")
    ...
    @command.argument(name="server_to_check", label="Server to check", required=True)
    async def version(self, command_event: MessageEvent, server_to_check: Optional[str]):
        if not server_to_check:
            await command_event.reply(
    ...
    async def _versions(
        ...
        async def _version_worker(queue: Queue) -> None:
            while True:
                ...
                try:
                    ...
                    server_to_version_data[worker_server_name] = FederationErrorResponse(
                        status_code=0,
        ...
        for server, result in server_to_version_data.items():
            ...
            if isinstance(result, FederationErrorResponse):
                server_software_col.maybe_update_column_width(len(str(result.status_code)))
                server_version_col.maybe_update_column_width(len(str(result.reason)))
            else:
                assert isinstance(result, FederationVersionResponse)
                server_software_col.maybe_update_column_width(len(result.server_software))
        ...
        for server_name in sorted_list_of_servers:
            ...
            if isinstance(server_data, FederationErrorResponse):
                # Pad software column with spaces, so error and code end up in version column
    ...
    async def _server_keys(
        ...
        for server_name, server_response in sorted(server_to_server_data.items()):
            ...
            if isinstance(server_response, FederationErrorResponse):
                ...
                old_verify_keys = server_response.response_dict.get("old_verify_keys", {})
                valid_until_ts: Optional[int] = server_response.response_dict.get(
                    "valid_until_ts", None
                )
                valid_until_pretty = "None Found"
                if valid_until_ts:
                    valid_until_pretty = str(datetime.fromtimestamp(float(valid_until_ts / 1000)))
                ...
                for key_id, _ in old_verify_keys.items():
                    ...
                    buffered_message += f"{server_key_col.pad(key_id)} | "
                    expired_ts: Optional[int] = old_verify_keys[key_id].get("expired_ts", None)
                    expired_ts_pretty = "None Found"
                    if expired_ts:
                        expired_ts_pretty = str(datetime.fromtimestamp(float(expired_ts / 1000)))
    ...
    async def _server_keys_from_notary(
        ...
        for server_name, server_results in server_to_server_data.items():
            for server_keys in server_results.response_dict.get("server_keys", []):
                ...
                if valid_until:
                    valid_until_pretty = str(datetime.fromtimestamp(float(valid_until / 1000)))
        ...
        for server_name, server_response in sorted(server_to_server_data.items()):
            ...
            if isinstance(server_response, FederationErrorResponse):
                ...
                for server_keys in server_response.response_dict.get("server_keys", []):
                    verify_keys = server_keys.get("verify_keys", {})
                    old_verify_keys = server_keys.get("old_verify_keys", {})
                    valid_until_ts: Optional[int] = server_keys.get("valid_until_ts", None)
                    valid_until_pretty = "None Found"
                    ...
                    for key_id in old_verify_keys.keys():
                        expired_valid_until: Optional[int] = old_verify_keys[key_id].get(
                            "expired_ts", None
                        )
                        expired_ts_pretty = "None Found"
                        if expired_valid_until:
                            expired_ts_pretty = str(
                                datetime.fromtimestamp(float(expired_valid_until / 1000))
                            )
                        buffered_message += f"{pad('', server_name_col.size)} | "
                        buffered_message += f"{server_key_col.pad(key_id)} | "
                        buffered_message += f"{expired_ts_pretty}\n"
    ...
    async def get_hosts_in_room_ordered(
        ...
        state_events = []
        for _, state_event in state_events_dict.items():
    ...
    async def _discover_event_ids_and_room_ids(
        ...
        if not event_id:
            ...
            if isinstance(ts_response, FederationErrorResponse):
                ...
                origin_server_ts = ts_response.response_dict.get("origin_server_ts", None)
        else:
...
def format_result_lines_var(line_segments: List[Tuple[str, int]]) -> str:
    buffered_message = ""
    for line_data, column_size in line_segments:  # pylint: disable=unused-variable
        pass
    return buffered_message
...
def make_into_text_event(message: str, ignore_body: bool = False) -> TextMessageEventContent:
    content = TextMessageEventContent(  # pylint: disable=unexpected-keyword-arg
...
def combine_lines_to_fit_event(list_of_all_lines: List[str], header_line: str) -> List[str]:
    """
